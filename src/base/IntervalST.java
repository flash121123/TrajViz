...
public class IntervalST<Value>  {
    ...
    ***************************************************************************/
    public boolean put(Interval1D interval, Value value) {
    	boolean flag=true;
        if (contains(interval)) { System.out.println("duplicate"); remove(interval);  flag=false;}
    ...
    ***************************************************************************/
	...
	private void remove(RuleInterval xs) {
		// TODO Auto-generated method stub
		Interval1D x=new Interval1D(xs.getStart(),xs.getEnd());
		if(this.contains(x))
			this.remove(x);
	}
	...
	public String toString() {
		return this.searchAll(new Interval1D(0,5000000)).toString();
	}
