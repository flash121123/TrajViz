...
public final class SequiturFactory {
  ...
  public static void updateRuleIntervals(GrammarRules rules, SAXRecords saxFrequencyData,
      boolean slidingWindowOn, double[] originalTimeSeries, int saxWindowSize, int saxPAASize) throws Exception {
    ...
    for (GrammarRuleRecord ruleContainer : rules) {
      ...
      for (Integer currentIndex : ruleContainer.getOccurrences()) {
        ...
        char[] temp=TSUtils.ts2String(TSUtils.optimizedPaa(Arrays.copyOfRange(originalTimeSeries,startPos,endPos),expandedRuleLength*6),TSUtils.send_c1());
      ...
      ruleContainer.setRuleInterval(resultIntervals);
