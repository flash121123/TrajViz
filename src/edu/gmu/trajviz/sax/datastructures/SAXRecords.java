...
import edu.gmu.trajviz.logic.JmotifMapEntry;
...
public class SAXRecords implements Iterable<SaxRecord> {
  ...
  // adapter to accept string as parameters   -qz
  public void add(String str, Integer idx){
	  SaxRecord rr = records.get(str);
	  if (null == rr){
		  rr = new SaxRecord(str.toCharArray(),idx);
		  this.records.put(str, rr);
	  }
	  else {
		  rr.addIndex(idx);
	  }
